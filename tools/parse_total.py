...
def parse_log_file(filepath):
    ...
    while i < len(lines):
        ...
        if match:
            ...
            cmpbytes_raw = match.group(3)
            cmpbyte_strs = [b.strip() for b in cmpbytes_raw.split(',')] if cmpbytes_raw else None
            ...
            key = (
                type_str,
                tuple(byte_strs),
                tuple(cmpbyte_strs) if cmpbyte_strs else None,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parse all DFSan logs with optional cmpbytes.")
    ...
    print(f"[ALL] 去重 + 排序后共输出 {len(parsed_data)} 个记录单元")
